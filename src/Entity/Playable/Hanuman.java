...
public class Hanuman extends Playable{

    Map tm;
    public Hanuman (Map tm){
        this.tm = tm;
        x = 50;
        y = 380;
        x_speed = 0;
        y_speed = 0;
        direction = KeyEvent.VK_KP_RIGHT;
        dead = false;

        try{
            icon_left = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/Entity/Playable/Hanuman.png")));
            icon_right = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/Entity/Playable/Hanuman.png")));
        }catch (IOException e){
            e.printStackTrace();
        }
    }
